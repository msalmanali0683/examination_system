...
<x-card class="max-w-3xl mx-auto">
    ...
    @if ($step === 'map')
        <div>
            ...
            <p class="mt-3 text-sm text-gray-500 dark:text-gray-400">Match your file's columns to the system fields. Room Name, Rows and Columns are required; the rest are optional.</p>
            ...
            <div class="mt-4 space-y-3">
                @foreach ($this->targetFields() as $field => $label)
                    <div class="flex items-center justify-between gap-4">
                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">
                            {{ $label }}
                            @if (in_array($field, ['name', 'rows', 'columns'])) <span class="text-red-500">*</span> @endif
                        </span>
                ...
                @endforeach
                @error('mapping.name') <span class="text-sm text-red-600 block">{{ $message }}</span> @enderror
                @error('mapping.rows') <span class="text-sm text-red-600 block">{{ $message }}</span> @enderror
                @error('mapping.columns') <span class="text-sm text-red-600 block">{{ $message }}</span> @enderror
            </div>
    ...
    @if ($step === 'review')
        <div>
            ...
            <div class="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
                ...
                <div class="p-3 bg-yellow-50 dark:bg-yellow-900/30 rounded-lg">
                    <div class="text-2xl font-semibold text-yellow-700 dark:text-yellow-300">{{ $report['duplicateNames'] }}</div>
                    <div class="text-xs text-gray-500 dark:text-gray-400">Duplicate names in file</div>
                </div>
            </div>
            ...
            <div class="mt-6 flex items-center gap-3">
                <x-btn wire:click="commitImport" icon="check">Import {{ $report['valid'] }} Room(s)</x-btn>
                <x-btn variant="ghost" wire:click="startOver">Start Over</x-btn>
            </div>
        </div>
    @endif
